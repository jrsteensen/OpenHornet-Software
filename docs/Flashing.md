# Flashing Firmware from Release Packages
[TOC]

This guide explains how to download pre-built firmware from an OpenHornet Software release package and flash it to your microcontrollers using free, open-source command-line tools. No Arduino IDE is required.

---

## Overview

Every tagged OpenHornet Software release includes a downloadable package named `OpenHornet-Software-<tag>.zip`. The package contains a `firmware/` folder with flashable firmware files generated by GitHub Actions from the tagged source.
...
2. Click **Releases**.
3. Open the release version you want.
4. Download `OpenHornet-Software-<tag>.zip` from the release assets.
5. Extract the zip.
6. Open the extracted `firmware/` folder. It contains `.hex`, `.eep`, and `.bin` files, one set per sketch.

Each firmware file is named after the sketch it belongs to (for example, `1A2-MASTER_ARM_PANEL.hex`). Use the table in [Sketch-to-Board Reference](#sketch-to-board-reference) to find the right file for your panel.
...
The release package contains a single merged binary (`.bin`) that includes the bootloader, partition table, and application in one file.
...
**Release package or firmware file not found**
Download the latest package from the GitHub Releases page and look inside its `firmware/` folder. If a release package is missing expected firmware, ask a maintainer to verify the release workflow or build the firmware locally — see the [Software Manual](SoftwareManual.md).
