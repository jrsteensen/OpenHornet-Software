...
/**
 * @brief  DCS-BIOS callback: update counter heartbeat.
 *
 * @details Called by the DCS-BIOS framework every time the simulator increments
 *          its internal update counter (once per exported-data frame, typically
 *          ~30 Hz).  The value is stored in @c currDcsHeartbeat; getDcsState()
 *          compares it against @c prevDcsHeartbeat to detect whether DCS is
 *          still streaming data.
 *
 * @param newValue  Latest value of the DCS-BIOS update counter (0-255, wraps).
 */
...
/**
 * @brief  DCS-BIOS callback: aircraft name string.
 *
 * @details Populated by DCS-BIOS with the loaded aircraft identifier
 *          (e.g. "FA-18C_hornet") or an empty string when no mission is
 *          active or DCS is in the main menu.  getDcsState() uses this to
 *          distinguish EXITED from all other states.
 *
 * @param newValue  Null-terminated aircraft name, up to 24 characters.
 *                  An empty string or nullptr indicates no active aircraft.
 */
...
/**
 * @brief  DCS-BIOS callback: left engine RPM percentage.
 *
 * @details Receives the IFEI left-engine RPM display string ("0" to "100").
 *          Converted to integer and stored in @c rpmLeft for use by
 *          getDcsState() to determine GROUND_HOT vs GROUND_COLD.
 *
 * @param newValue  Engine RPM as a decimal ASCII string ("0"-"100").
 *                  nullptr or empty string is treated as 0 RPM.
 */
...
/**
 * @brief  DCS-BIOS callback: right engine RPM percentage.
 *
 * @details Receives the IFEI right-engine RPM display string ("0" to "100").
 *          Converted to integer and stored in @c rpmRight for use by
 *          getDcsState() to determine GROUND_HOT vs GROUND_COLD.
 *
 * @param newValue  Engine RPM as a decimal ASCII string ("0"-"100").
 *                  nullptr or empty string is treated as 0 RPM.
...
/**
 * @brief  DCS-BIOS callback: weight-on-wheels sensor, left main gear.
 *
 * @details 1 = gear is on the ground, 0 = gear is off the ground (airborne).
 *          getDcsState() returns AIRBORNE only when both wowLeft and wowRight
 *          are 0.
 *
 * @param newValue  Weight-on-wheels state: 1 (ground) or 0 (air).
 */
...
/**
 * @brief  DCS-BIOS callback: weight-on-wheels sensor, right main gear.
 *
 * @details 1 = gear is on the ground, 0 = gear is off the ground (airborne).
 *          getDcsState() returns AIRBORNE only when both wowLeft and wowRight
 *          are 0.
