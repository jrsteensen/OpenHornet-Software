...
$(MERGED_BIN): all
	...
	test -f "$(BOOT_APP_BIN)"
	test -f "$(MAIN_EXE)"
	$(ESPTOOL) --chip $(MCU) merge-bin -o $@ --pad-to-size $(FLASH_DEF) --flash-mode keep --flash-freq keep --flash-size keep $(BOOTLOADER_ADDR) "$(BOOTLOADER_BIN)" $(PARTITIONS_ADDR) "$(PARTITIONS_BIN)" $(BOOT_APP_ADDR) "$(BOOT_APP_BIN)" $(APP_ADDR) "$(MAIN_EXE)"
